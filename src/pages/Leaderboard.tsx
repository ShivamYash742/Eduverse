...
import { topStudents, getStudentsByField, getAllFields, Student } from "@/lib/leaderboardData";
import { Trophy, Medal, Search, Filter, ArrowUpDown, Mail, UserPlus } from "lucide-react";
import { useUser } from "@/contexts/UserContext";
import Navbar from "@/components/Navbar";

const Leaderboard = () => {
  const [selectedField, setSelectedField] = useState("All Fields");
  const [filteredStudents, setFilteredStudents] = useState<Student[]>(topStudents);
  const [searchQuery, setSearchQuery] = useState("");
  const [sortDirection, setSortDirection] = useState<"asc" | "desc">("desc");
  const [viewMode, setViewMode] = useState<"leaderboard" | "recruiter">("leaderboard");
  const { user } = useUser();
  const allFields = ["All Fields", ...getAllFields()];

  // Effect to filter students based on selected field and search query
  useEffect(() => {
    let students = selectedField === "All Fields" 
      ? [...topStudents]
    ...
    students = students.sort((a, b) => {
      const comparison = sortDirection === "desc" ? b.points - a.points : a.points - b.points;
      return comparison;
    });
        ...
        <div className="flex flex-col space-y-6">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            ...
            {user?.role === "recruiter" || user?.role === "student" ? (
              <Tabs value={viewMode} onValueChange={value => setViewMode(value as "leaderboard" | "recruiter")}>
                <TabsList>
          ...
          <Card className="bg-card/60 backdrop-blur-sm border-accent">
            <CardHeader className="pb-4">
              <div className="flex flex-col md:flex-row justify-between md:items-center gap-4">
                <div>
                  <CardTitle>
                    {viewMode === "leaderboard" ? "Top Students" : "Talent Pool"}
                  </CardTitle>
                  <CardDescription>
                    {viewMode === "leaderboard" 
                  ...
                  {viewMode === "leaderboard" ? (
                      ...
                      <table className="w-full text-left">
                        <thead>
                          <tr className="border-b border-accent">
                            ...
                            <th className="px-4 py-3 text-sm font-medium text-muted-foreground">Field</th>
                            <th className="px-4 py-3 text-sm font-medium text-muted-foreground">Points</th>
                            <th className="px-4 py-3 text-sm font-medium text-muted-foreground">Badges</th>
                          </tr>
                        </thead>
                        <tbody>
                          {filteredStudents.map((student, index) => (
                            <tr key={student.id} className="border-b border-accent/50 hover:bg-accent/10">
                              ...
                              <td className="px-4 py-4">
                                <div className="space-y-1">
                                  <div className="font-medium">{student.points.toLocaleString()}</div>
                                  <Progress value={student.points / 100} className="h-1" />
                                </div>
                              </td>
                              <td className="px-4 py-4">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                      {filteredStudents.map((student) => (
                        <Card key={student.id} className="bg-accent/20 border-accent/50 overflow-hidden">
                          ...
                          <div className="p-6 pt-0 space-y-4">
                            ...
                            <div className="grid grid-cols-3 gap-2 text-center">
                              <div className="bg-accent/30 rounded-md p-2">
                                <p className="text-xl font-semibold">{student.points}</p>
                                <p className="text-xs text-muted-foreground">Points</p>
                              </div>
                              <div className="bg-accent/30 rounded-md p-2">
                                <p className="text-xl font-semibold">{student.projects}</p>
                                <p className="text-xs text-muted-foreground">Projects</p>
                              </div>
                              <div className="bg-accent/30 rounded-md p-2">
                                <p className="text-xl font-semibold">{student.quizzes}</p>
